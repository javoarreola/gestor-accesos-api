...
def obtener_empresa(id_empresa: int, db: Session = Depends(get_db)):
    empresa = empresa_service.obtener_empresa_por_id(db, id_empresa)
...
def crear_empresa(empresa: EmpresaCreate, db: Session = Depends(get_db)):
    return empresa_service.crear_empresa(db, empresa)
...
def actualizar_empresa(
    ...
    empresa_actualizada = empresa_service.actualizar_empresa(
        db,
        id_empresa,
        empresa
    )
...
def eliminar_empresa(id_empresa: int, db: Session = Depends(get_db)):
    empresa_eliminada = empresa_service.eliminar_empresa(db, id_empresa)
