...
def get_by_id(db: Session, id_empresa: int):
    return db.query(Empresa).filter(Empresa.id_empresa == id_empresa).first() #busqueda de empresas, igualmente se busca comparando ids
...
def create(db: Session, empresa: EmpresaCreate):
    ...
    return nueva_empresa #se crea una empresa
...
def update(db: Session, id_empresa: int, empresa: EmpresaUpdate):
    ...
    return empresa_db #se actualiza una empresa
...
def delete(db: Session, id_empresa: int):
    ...
    return empresa_db #se eliminar una empresa